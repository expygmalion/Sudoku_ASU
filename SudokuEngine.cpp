...
void printScore(int elapsedSeconds, int difficultyOffset, int& userScore) {
    if (difficultyOffset == 2) {
     
        userScore = difficultyOffset * 27 * (0.1*elapsedSeconds);
    } else if (difficultyOffset == 54) {

        userScore = difficultyOffset * 81 * (0.1 * elapsedSeconds);
    } else if (difficultyOffset == 63) {
    
        userScore = difficultyOffset * 729 * (0.1*elapsedSeconds);
    } else {

              userScore = difficultyOffset * difficultyOffset * (0.1*elapsedSeconds);
    }
    cout << userScore;
}
...
void endGame(int sudokuGrid[GRID_SIZE][GRID_SIZE], int comparsionGrid[GRID_SIZE][GRID_SIZE],
            ...
            int &userScore, int difficultyOffset)
{
    ...
    if (difficultyOffset == 2) {
        userScore = difficultyOffset * 50 * elapsedSeconds;
    } else if (difficultyOffset == 54) {
        userScore = difficultyOffset * 150 * elapsedSeconds;
    } else if (difficultyOffset == 63) {
        userScore = difficultyOffset * 200 * elapsedSeconds;
    } 
    // Update player data
...
void confirmSolution(int sudokuGrid[GRID_SIZE][GRID_SIZE], int comparsionGrid[GRID_SIZE][GRID_SIZE], bool &allCellsFilled, bool &solutionValidated, bool &puzzleSolved, int elapsedSeconds, int &userScore, int difficultyOffset)
{
    endGame(sudokuGrid, comparsionGrid, allCellsFilled, solutionValidated, puzzleSolved, elapsedSeconds, userScore, difficultyOffset);

}

// Function to print the Sudoku grid
void printGrid(int grid[GRID_SIZE][GRID_SIZE], int difficultyOffset, bool localize[GRID_SIZE][GRID_SIZE])
{
    // Delete elements randomly
    srand(time(0));

    for (int k = 0; k < difficultyOffset; ++k)
    {
...
int main()
{
    ...
    printGrid(sudokuGrid, difficultyOffset, emptyCells);
    ...
    while (!puzzleSolved)
    {
        ...
        printScore(elapsedSeconds, difficultyOffset, userScore);
    ...
    case 'c':
        confirmSolution(sudokuGrid, comparsionGrid, allCellsFilled, solutionValidated, puzzleSolved, elapsedSeconds, userScore, difficultyOffset);
        break;
    case 'E':
    case 'e':
